...
class TestOutboundMessageResponse(unittest.TestCase):
    def test_outbound_message_response(self) -> None:
        omr = OutboundMessageResponse(version=Version(2100099), request_id=42)
    ...
    def test_outbound_message_response_stream(self) -> None:
        ...
        self.assertEqual(
            out.getvalue(),
            b"ES\x00\x00\x00\x18\x00\x00\x00\x00\x00\x00\x00\x02\x09\x08\x2d\xc7\x23\x00\x00\x00\x02" + b"\x00\x00" + b"\x04test",  # tcp header  # context  # transport message
        )
        self.assertEqual(
            len(out.getvalue()),
